...
function createHealthGroup(name, members, options, probeUrl) {
    ...
    return {
        name,
        type: leastPing ? 'url-test' : 'fallback',
        proxies: members,
        url: probeUrl,
...
function replacePolicyInRule(rule, policyRenames) {
    ...
    for (let index = 1; index < parts.length; index += 1) {
        parts[index] = policyRenames.get(parts[index]) || parts[index];
    }
    return parts.join(',');
}
...
function transformMihomoDocument(document, options = {}) {
    ...
    for (const name of KNOWN_AUTO_GROUP_NAMES) policyRenames.set(name, firstPolicy);
    if (fastestName !== firstPolicy) policyRenames.set(fastestName, firstPolicy);
    const referenceRenames = new Map([...proxyRenames, ...policyRenames]);

    const preservedGroups = [];
    for (const group of proxyGroups) {
        if (!group || typeof group !== 'object' || typeof group.name !== 'string') continue;
        if (KNOWN_AUTO_GROUP_NAMES.has(group.name) || group.name === fastestName) continue;

        const originalMembers = Array.isArray(group.proxies) ? group.proxies : [];
        const hadRemoteMembers = originalMembers.some((member) => remoteNames.has(member));
        const nextMembers = [];
        const seen = new Set();
        ...
        for (const member of originalMembers) {
            if (remoteNames.has(member)) continue;
            addMember(policyRenames.get(member) || proxyRenames.get(member) || member);
        }
        if (hadRemoteMembers) {
            if (fastestGroup) addMember(fastestGroup.name);
            for (const member of selectorEntries) addMember(member);
        }

        group.proxies = nextMembers;
        if (typeof group['default-selected'] === 'string') {
            group['default-selected'] = policyRenames.get(group['default-selected']) || group['default-selected'];
        }
