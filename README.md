...
| `sticky_enabled` | boolean | Закреплять fastest-group за одной нодой по token | `true` |
...
Для случая, когда у пользователя под блокировками реально работает только одна нода из `Самые быстрые`, добавлен `sticky by token`.

Как это работает:

1. Пользователь впервые получает fastest-group.
2. Middleware выбирает одну ноду из текущего fastest-пула.
3. Привязка `token -> node` сохраняется на `sticky_ttl_sec`.
4. Пока TTL жив, fastest-group для этого пользователя будет содержать только эту ноду.
5. Если нода исчезла из доступного fastest-пула, middleware автоматически выберет новую и перепривяжет token.

Это убирает дергание между 3-4 нодами и делает fastest-group стабильнее для Happ и похожих клиентов.
...
Рекомендуемое значение для мобильных пользователей под блокировками: `sticky_ttl_sec: 1800..3600`.
...
2. Проверьте `sticky.assigned_node` через `GET /admin/debug/token/{token}`
3. При необходимости увеличьте `sticky_ttl_sec`
