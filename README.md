...
- **Может закреплять** пользователя за одной рабочей нодой через `sticky`
...
| `sticky_*` | — | ✅ | Sticky-привязка fastest-group по token |
...
| `fastest_probe_url` | string | Отдельный URL probe только для fastest-group | `probe_url` |
...
| `sticky_enabled` | boolean | Включить sticky-логику для fastest-group | `true` |
| `sticky_mode` | string | `pin` = одна нода, `prefer` = preferred node первой в пуле | `pin` (для блокировок обычно лучше `prefer`) |
| `sticky_ttl_sec` | number | Сколько держать sticky-привязку | `3600` (для блокировок обычно `60..600`) |
| `sticky_max_entries` | number | Лимит sticky-записей в памяти | `10000` |
...
Для `🏁 Самые быстрые` можно задать отдельный `fastest_probe_url`.

Это полезно под региональными блокировками:
- обычные страны продолжают пинговаться через глобальный `probe_url`;
- fastest-group можно пинговать через локально доступный сайт, например `https://ya.ru`;
- тогда клиент будет считать "живой" ту ноду, через которую реально проходит этот probe.
...
Для fastest-group есть `sticky by token` с двумя режимами.

#### `sticky_mode = pin`

- Пользователь впервые получает fastest-group.
- Middleware выбирает одну ноду из текущего fastest-пула.
- Пока TTL жив, fastest-group для этого пользователя будет содержать только эту ноду.

Это самый жесткий режим. Он хорошо убирает дергание, но мешает client-side observatory переоценивать весь fastest-пул.

#### `sticky_mode = prefer`

- Middleware запоминает preferred node по token.
- В fastest-group возвращается весь пул нод.
- Preferred node просто ставится первой в списке.
- Если preferred node исчезла из fastest-пула, middleware выбирает новую.

Это "умный sticky":
- warmup после refresh остается стабильнее;
- client-side observatory всё ещё видит весь пул;
- under-blocking клиент может переключиться на рабочую ноду по probe, а не застревать на pinned server.
...
Для мобильных пользователей под блокировками обычно безопаснее `sticky_mode: "prefer"` и `sticky_ttl_sec: 60..600`.
...
| `GET /admin/quarantine` | Список quarantined нод (admin) |
| `POST /admin/quarantine` | Добавить ноду в quarantine (admin) |
| `DELETE /admin/quarantine/{node}` | Удалить ноду из quarantine (admin) |
...
Если клиент under-blocking прыгает между нодами:
1. Включите `sticky_enabled`
2. Для fastest-group под блокировками предпочтительнее `sticky_mode = prefer`, а не `pin`
3. Проверьте `sticky.assigned_node` через `GET /admin/debug/token/{token}`
4. При необходимости увеличьте `sticky_ttl_sec`
